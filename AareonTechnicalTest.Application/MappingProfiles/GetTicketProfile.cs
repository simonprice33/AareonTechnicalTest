    ...
    public class GetTicketProfile : Profile
    {
        public GetTicketProfile()
        {
            CreateMap<Ticket, GetTicketResponse>()
                ...
                .ForMember(dest => dest.LastUpdatedBy, opt => opt.MapFrom(src =>
                         $"{src.Person.Forename ?? string.Empty} {src.Person.Surname ?? string.Empty}"))
                .ForMember(dest => dest.UpdatedAt, opt => opt.MapFrom(src => src.UpdatedDateTime));
        }
    }
}